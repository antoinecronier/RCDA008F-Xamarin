...
namespace Tp4.Models
{
    public class LoginForm
    {
        ...
        public LoginForm(Entry login, Entry password, Xamarin.Forms.Switch isRemind, View loginForm, View tweetForm, Label errorLabel, Button button, StackLayout tweetList)
        {
            ...
            TweetFormLoader(tweetList);
        }

        private void TweetFormLoader(StackLayout tweetForm)
        {
            tweetForm.Children.Clear();
            foreach (var item in twitterService.Tweets)
            {
                tweetForm.Children.Add(new Views.Tweet().LoadData(item));
            }
        }
